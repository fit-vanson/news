...
//			->join('languages', 'brands.lan', '=', 'languages.language_code')
			->select('brands.id', 'brands.name', 'brands.thumbnail', 'brands.is_featured', 'brands.is_publish', 'tp_status.status'
//                , 'languages.language_name'
            )
			->orderBy('brands.id','desc')
			->paginate(10);

        return view('backend.brands', compact('media_datalist',  'statuslist', 'datalist'));
    }
...
//			'lan' => $lan,
    ...
    //Save data for Brands Bulk
    public function saveBrandsBulk(Request $request){
        ...
        $brands = [];
        foreach ($rows as $row) {
            $existingBrand = Brand::where('name', $row[0])->first();
