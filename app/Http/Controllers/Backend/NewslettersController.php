...
class NewslettersController extends Controller
{
    ...
    public function getSubscribeSettings(){
        ...
        $data = [
            'id' => $id,
            'title_row' => 'Subscribe Popup',
            'name' => $site->site_name,
            'web' => $site->site_web,

            'subscribe_title' => '',
            'subscribe_popup_desc' => '',
            'bg_image_popup' => '',
            'background_image' => '',
            'is_subscribe_popup' => '',
            'is_subscribe_footer' => '',
        ...
        if(isset($site_option)){
            $results = $site_option->subscribe_popup;
            if($results){
                $dataObj = json_decode($results);
                $data['subscribe_title'] = $dataObj->subscribe_title;
                $data['subscribe_popup_desc'] = $dataObj->subscribe_popup_desc;
                $data['bg_image_popup'] = $dataObj->bg_image_popup;
                $data['background_image'] = $dataObj->background_image;
                $data['is_subscribe_popup'] = $dataObj->is_subscribe_popup;
                $data['is_subscribe_footer'] = $dataObj->is_subscribe_footer;
            }
        }
		$datalist = $data;
    ...
    public function SubscribePopupUpdate(Request $request){
		...
		$data = array(
			'site_id' => $site_id,
			'subscribe_popup' => json_encode($option_value)
		);
