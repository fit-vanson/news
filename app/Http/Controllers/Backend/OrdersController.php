...
//			->join('users as c', 'a.seller_id', '=', 'c.id')
			...
			->select('a.id', 'a.customer_id', 'a.payment_status_id', 'a.order_status_id', 'a.order_no', 'a.created_at', 'a.shipping_fee', DB::raw("SUM(g.total_price) as total_amount"), DB::raw("SUM(g.tax) as tax"),
//                'b.name', 'c.shop_name',
                'd.method_name', 'e.pstatus_name', 'f.ostatus_name')
			->groupBy('a.customer_id', 'a.payment_status_id', 'a.order_status_id', 'a.created_at', 'f.ostatus_name', 'e.pstatus_name', 'd.method_name', 'a.shipping_fee',
//                'b.name', 'c.shop_name',
                'a.order_no', 'a.id')
...
//                'c.shop_name',
            ...
            ->orderBy(
                ...
                'desc')
            ->paginate(25);
//        dd($datalist,DB::table('order_masters')->get());
...
//			->join('users as c', 'a.seller_id', '=', 'c.id')
			...
			->join('order_status as f', 'a.order_status_id', '=', 'f.id')
			->join('order_items as g', 'a.id', '=', 'g.order_master_id')
			->join('multiple_sites as h', 'a.site_id', '=', 'h.id')
			->select(
				'a.id',
                'a.site_id',
                'h.site_name',
...
//				'c.email',
		...
		foreach($datalist as $row){
			...
			if($gtext['currency_position'] == 'left'){
				$price = $gtext['currency_icon'].number_format($row->price,2);
				$total_price = $gtext['currency_icon'].number_format($row->total_price,2);
			}else{
				$price = number_format($row->price,2).$gtext['currency_icon'];
				$total_price = number_format($row->total_price,2).$gtext['currency_icon'];
			}
