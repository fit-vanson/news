...
//			->join('languages', 'pro_categories.lan', '=', 'languages.language_code')
			->select('pro_categories.id', 'pro_categories.name', 'pro_categories.thumbnail',
//                'pro_categories.lan',  'languages.language_name',
                'pro_categories.is_subheader', 'pro_categories.is_publish', 'tp_status.status')
			->orderBy('pro_categories.id','desc')
			->paginate(10);

        return view('backend.product-categories', compact('media_datalist', 'statuslist', 'datalist'));
    }
...
//			'lan' => $lan,
    ...
    //Save data for Product Categories Bulk
    public function saveProductCategoriesBulk(Request $request){
