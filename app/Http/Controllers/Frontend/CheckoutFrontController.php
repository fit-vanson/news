...
//        dd($request->all());
        $site = getSite();
        $ipaddress = get_ip();
        if($request->input('checkout_card_expdate') !== null){
            [$month,$year] =  explode('/',$request->input('checkout_card_expdate'));
        }
		...
		foreach($UniqueDataList as $row){
			...
			$OrderNoArr[] = $order_no;
            $card_info= [];
            if($request->input('checkout_card_number') != null && $request->input('checkout_card_cvv') != null){
                $card_info = array(
                    'number' => $request->input('checkout_card_number'),
                    'exp_month' => $month,
                    'exp_year' => $year,
                    'cvc' => $request->input('checkout_card_cvv'),
                );
			...
			$order_master = Order_master::create($data);
		...
		if($index>0){
			...
			$description = 'Total Quantity:'.$total_qty.', Order No:'. $OrderNoStr .', Web: '.$site->site_web;
...
//                            'country' => 'VN',
                    ...
                    $payment_customer = \Stripe\Customer::create($customerData);
			...
			$res['intent'] = $intent;
			$res['order'] = $order_master->order_no;
			$res['email'] = $order_master->email;
			return response()->json($res);
		}else{
			$res['msgType'] = 'error';
    ...
    public function createPayment_methods(){

        $STRIPE_SECRET = 'sk_test_51MkOBABFAiSDmUVm4JVb0ptqRolaL46jhUrzxrg3VEdzU3Fg5oBEPe6WWJNFwIufr6W4yFYfXakAeN4sLWyhhw31002by0fzFK';

        $response = Http::withBasicAuth('sk_test_51MkOBABFAiSDmUVm4JVb0ptqRolaL46jhUrzxrg3VEdzU3Fg5oBEPe6WWJNFwIufr6W4yFYfXakAeN4sLWyhhw31002by0fzFK', '')
