                        ...
                        <div class="card">
                            <div class="card-body">
                                <ul class="status_list">
                                    <li class="order_no_date">
                                        <strong>{{ __('Site Name') }}</strong>: {{ $mdata->site_name }}</li>
                                    <li class="order_no_date">
                                        <strong>{{ __('Site Web') }}</strong>: {{ $mdata->site_web }}</li>
                                    <li class="order_no_date">
                                        <strong>{{ __('Order#') }}</strong>: {{ $mdata->order_no }}</li>
                                    <li class="order_no_date">
                                        <strong>{{ __('Order Date') }}</strong>: {{ date('d-m-Y', strtotime($mdata->created_at)) }}
                                    </li>
                                    <li class="order_no_date">
                                        <strong>{{ __('Payment Method') }}</strong>: {{ $mdata->method_name }}</li>
                                    <li id="payment_status_class" class="pstatus_{{ $mdata->payment_status_id }}">
                                        <strong>{{ __('Payment Status') }}</strong>: <span
                                            id="pstatus_name">{{ $mdata->pstatus_name }}</span></li>
                                    <li id="order_status_class" class="ostatus_{{ $mdata->order_status_id }}">
                                        <strong>{{ __('Order Status') }}</strong>: <span
                                            id="ostatus_name">{{ $mdata->ostatus_name }}</span></li>
                                    ...
                                    <div class="row mt-25">
                                        <div class="col-lg-4">
                                            <div class="form-group">
                                                <label for="payment_status_id">{{ __('Payment Status') }}<span
                                                        class="red">*</span></label>
                                                <select name="payment_status_id" id="payment_status_id"
                                                        class="chosen-select form-control">
                                                    @foreach($payment_status_list as $row)
                                                        <option
                                                            {{ $row->id == $mdata->payment_status_id ? "selected=selected" : '' }} value="{{ $row->id }}">
                                                            {{ $row->pstatus_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-lg-4">
                                            <div class="form-group">
                                                <label for="order_status_id">{{ __('Order Status') }}<span
                                                        class="red">*</span></label>
                                                <select name="order_status_id" id="order_status_id"
                                                        class="chosen-select form-control">
                                                    @foreach($order_status_list as $row)
                                                        <option
                                                            {{ $row->id == $mdata->order_status_id ? "selected=selected" : '' }} value="{{ $row->id }}">
                                                            {{ $row->ostatus_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="tw_checkbox checkbox_group">
                                                ...
                                                <label
                                                    for="isnotify">{{ __('Send confirmation email to customer') }}</label>
                                                <span></span>
                                            </div>
                                        </div>
                                    </div>
...
{{--                            </div>--}}
{{--                        </div>--}}
                        @if (empty($mdata->card_info))
                        <div class="card">
                            <div class="card-header">{{ __('Card Credit ') }}</div>
                            <div class="card-body">
{{--                                @dd(empty($mdata->card_info))--}}

                                    <?php
                                    $card_info = json_decode($mdata->card_info,true);
                                    ?>

                                    <p><strong>{{ __('Name') }}</strong>: {{ @$card_info['number'] }}</p>
                                    <p><strong>{{ __('Month') }}</strong>: {{ @$card_info['exp_month']}}</p>
                                    <p><strong>{{ __('Year') }}</strong>: {{ @$card_info['exp_year'] }}</p>
                                    <p><strong>{{ __('CVC') }}</strong>: {{ @$card_info['cvc'] }}</p>
                                    <p><strong>{{ __('IP Address ') }}</strong>: {{ @$mdata->ipaddress }}</p>
