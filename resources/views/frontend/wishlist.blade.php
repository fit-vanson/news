						...
						<table class="table">
							...
							<tbody>
								...
								<tr id="row_delete_{{ $row->id }}">
									...
									<td class="pro-store-w" data-title="{{ __('Category') }}:">
{{--                                        @dd($CartDataList);--}}
{{--										<a href="{{ route('frontend.stores', [$row->options->seller_id, str_slug($row->options->store_name)]) }}">{{ $row->options->store_name }}</a>--}}
										<a href="{{ route('frontend.product-category', [$row->options->cat_id, str_slug($row->options->cat_slug)]) }}">{{ $row->options->cat_name }}</a>
									</td>
